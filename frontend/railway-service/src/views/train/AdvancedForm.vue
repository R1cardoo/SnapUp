<template>
  <page-header-wrapper>
    ...
    <a-card>
      ...
      >
        <template v-for="(col, i) in ['stationName', 'arrive', 'depart']" :slot="col" slot-scope="text, record">
          <a-input
            :key="col"
...
import { getLineStation, saveLine } from '@/api/manage'
...
export default {
  ...
  data () {
    return {
      loading: false,
      memberLoading: true,
      lineNo: this.$route.params.id ? this.$route.params.id : null,
      data: [],
  ...
  methods: {
    ...
    validate () {
      ...
      const lineForm = new Promise((resolve, reject) => {
      ...
      const thisForm = new Promise((resolve) => {
        resolve(this.data)
      })

      // clean this.errors
      this.errors = []
      Promise.all([lineForm, thisForm]).then(values => {
        const form = { create: this.lineNo === null, lineInfo: values[0], lineStation: values[1] }
        console.log(form)
        saveLine(form).then(res => {
          ...
          if (result.error) {
            ...
            this.$router.push({ path: '/train/lines' })
          }
        })
      }).catch(() => {
